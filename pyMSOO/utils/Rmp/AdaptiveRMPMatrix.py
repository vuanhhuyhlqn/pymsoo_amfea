...
class IndividualRMP:
    def __init__(self, strategy, crossover, mutation):
    ...
    def calc_diff(self, rmp_matrix, p1_genes, p2_genes, p1_skill_factor, p2_skill_factor, p1_fitness, p2_fitness, tasks):
        ...
        for i in range(num_pair):
            ...
            if x1_skill_factor == x2_skill_factor or np.random.rand() < rmp_matrix[x1_skill_factor][x2_skill_factor]:
                #crossover
                oa, ob = self.crossover(Individual(x1, x1_skill_factor, x1_fitness), 
                                        Individual(x2, x2_skill_factor, x2_fitness), 
                                        x1_skill_factor,
                                        x2_skill_factor)
                oa.fcost = oa.eval(tasks[oa.skill_factor])
                ob.fcost = ob.eval(tasks[ob.skill_factor])
                assert(oa.fcost is not None)
                assert(ob.fcost is not None)
...
class PopulationRMP:
    def __init__(self, pop_size, crossover, mutation):
    ...
    def gen_pop(self, collect_state, p1, p2, p1_skill_factor, p2_skill_factor, p1_fitness, p2_fitness, tasks):
        for _ in range(self.pop_size):
            ...
            individual = IndividualRMP(strategy, self.crossover, self.mutation)
            # individual.evaluate(collect_state, p1, p2, p1_skill_factor, p2_skill_factor, p1_fitness, p2_fitness, tasks)
            self.individuals.append(individual)

class AdaptiveRMP:
    def __init__(self, rmp_pop_size, num_gen, pc, pm, crossover, mutation):
        self.rmp_pop_size = rmp_pop_size
        self.rmp_pop = PopulationRMP(self.rmp_pop_size, crossover, mutation)
        ...
        if gen_mfea % llm_rate == 0:
            ...
            for _ in range(self.num_gen):
                ...
                if np.random.rand() < self.pc:
                    ...
                    crossover_individual = IndividualRMP(off_strategy, self.crossover, self.mutation)
                    ...
                    if crossover_individual.performance >= par1.performance or crossover_individual.performance >= par2.performance:
                        ...
                        reversed_individual = IndividualRMP(off_strategy, self.crossover, self.mutation)
                    ...
                    if np.random.rand() < self.pm:
                        ...
                        mutation_individual = IndividualRMP(off_strategy, self.crossover, self.mutation)
